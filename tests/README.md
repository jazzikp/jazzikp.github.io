...
| File | Checks |
| --- | --- |
| `build.test.mjs` | Every page returns 200; generated files exist; the stylesheet is compiled and minified; `_sass/` and `img/src/` stay unpublished; the service worker's cache version matches `asset_version` and everything it precaches is real; sitemap, robots, feed and manifest are well formed. |
